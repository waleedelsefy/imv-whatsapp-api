...
class IMV_API_Login_Form_Manager {
    ...
    public function enqueue_scripts() {
        // Only load these on the My Account page when the user is not logged in.
        if ( function_exists('is_account_page') && is_account_page() && ! is_user_logged_in() ) {
            // Create a 'public' folder in your plugin directory for these assets.
            wp_enqueue_style( 'imv-otp-login-css', plugin_dir_url( __DIR__ ) . 'public/css/imv-otp-login.css', array(), '1.0.0' );

            wp_enqueue_script( 'imv-otp-login-js', plugin_dir_url( __DIR__ ) . 'public/js/imv-otp-login.js', array('jquery'), '1.0.0', true );
    ...
    public function ajax_verify_otp_and_login() {
        ...
        // Verify OTP by reusing the core logic
        $verification_response = IMV_API_OTP_Manager::handle_otp_verification(new WP_REST_Request('POST', ['phone' => $phone, 'otp' => $otp]));
