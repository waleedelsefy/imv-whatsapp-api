...
/**
 * This function handles the logic for the payment page. It runs on every page load
 * and checks if the current page is the order payment page.
 */
function imv_api_allow_guest_payment_for_order( $is_required ) {
	// If we are on the order payment page and a valid payment key is present in the URL,
	// then login is not required. This allows anyone with the link to pay,
	// respecting the "guest checkout" setting even for orders assigned to a user.
...
final class IMV_WhatsApp_API_Main {
	...
	private function define_hooks() {
		...
		$this->loader->add_filter( 'manage_users_columns', $this->admin, 'add_wallet_balance_column' );
		$this->loader->add_filter( 'manage_users_custom_column', $this->admin, 'display_wallet_balance_column', 10, 3 );
		$this->loader->add_filter( 'manage_users_sortable_columns', $this->admin, 'make_wallet_column_sortable' );
		$this->loader->add_action( 'pre_get_users', $this->admin, 'sort_users_by_wallet_balance' );
	}
}
